...
// Helper to send email via SendGrid (works perfectly on Render!)
const sendViaSendGrid = async (to, subject, html, attachments = []) => {
    ...
    try {
        const msg = {
            to: to,
            from: 'info.project.medisync@gmail.com', // Your verified sender email
            subject: subject,
            html: html,
...
const sendEmail = async (to, subject, html, attachments = []) => {
    // Try SendGrid first (works best on Render)
    const sendgridResult = await sendViaSendGrid(to, subject, html, attachments);
    if (sendgridResult && sendgridResult.success) return sendgridResult;
    
    // Log that email failed but don't block the main flow
    console.log(`⚠️ Could not send email to ${to}, but continuing...`);
    return { success: false, provider: 'none', error: 'No email method available' };
};
...
const sendStockUpdateNotification = async (userEmail, userName, medicineName, strength, hospitalName, district, newQuantity) => {
    try {
        ...
        const result = await sendViaSendGrid(userEmail, `📦 Stock Update Alert: ${medicineName} is Now Available!`, html);
        if (result && result.success) {
            console.log(`✅ Email sent to ${userEmail} for ${medicineName}`);
            return { success: true };
        } else {
            console.log(`⚠️ Could not send email to ${userEmail}`);
    ...
    try {
        ...
        for (const admin of admins) {
            try {
                ...
                const result = await sendViaSendGrid(admin.email, `📦 New Stock Arrival Request - ${requestData.hospitalName}`, html);
                if (result && result.success) {
                    console.log(`✅ Stock arrival request email sent to admin ${admin.email}`);
                    results.push({ email: admin.email, success: true });
                } else {
                    results.push({ email: admin.email, success: false, error: 'SendGrid failed' });
                }
            } catch (adminError) {
...
const sendStockArrivalApprovedToManager = async (managerEmail, managerName, stockRequest, approvedMedicine, adminUser, updatedQuantity) => {
    try {
        ...
        const result = await sendViaSendGrid(managerEmail, `✅ Stock Arrival Approved - ${approvedMedicine.medicineName}`, html);
        if (result && result.success) {
            console.log(`✅ Stock approval email sent to manager ${managerEmail}`);
            return { success: true };
        } else {
            console.log(`⚠️ Could not send stock approval email to ${managerEmail}`);
            return { success: false, error: 'SendGrid failed' };
        }
    } catch (error) {
...
const sendStockArrivalRejectedToManager = async (managerEmail, managerName, stockRequest, rejectedMedicine, adminUser, rejectionReason) => {
    try {
        ...
        const result = await sendViaSendGrid(managerEmail, `❌ Stock Arrival Rejected - ${rejectedMedicine.medicineName}`, html);
        if (result && result.success) {
            console.log(`✅ Stock rejection email sent to manager ${managerEmail}`);
            return { success: true };
        } else {
            console.log(`⚠️ Could not send stock rejection email to ${managerEmail}`);
            return { success: false, error: 'SendGrid failed' };
        }
    } catch (error) {
...
const sendTransferRequestEmail = async (toHospitalManager, fromHospital, toHospital, medicines, requestId) => {
    try {
        ...
        const result = await sendViaSendGrid(toHospitalManager.email, `📋 New Transfer Request from ${fromHospital.name}`, html);
        if (result && result.success) {
            console.log(`✅ Transfer request email sent to ${toHospitalManager.email}`);
            return { success: true };
        } else {
            console.log(`⚠️ Could not send transfer request email to ${toHospitalManager.email}`);
            return { success: false, error: 'SendGrid failed' };
        }
    } catch (error) {
...
const sendTransferApprovedEmail = async (requestingHospitalManager, fromHospital, toHospital, medicines, requestId) => {
    try {
        ...
        const result = await sendViaSendGrid(requestingHospitalManager.email, `✅ Transfer Request Approved - ${requestId}`, html);
        if (result && result.success) {
            console.log(`✅ Transfer approved email sent to ${requestingHospitalManager.email}`);
            return { success: true };
        } else {
            console.log(`⚠️ Could not send transfer approved email to ${requestingHospitalManager.email}`);
            return { success: false, error: 'SendGrid failed' };
        }
    } catch (error) {
...
const sendTransferCompletedEmail = async (sendingHospitalManager, fromHospital, toHospital, medicines, requestId) => {
    try {
        ...
        const result = await sendViaSendGrid(sendingHospitalManager.email, `✅ Transfer Completed - ${requestId}`, html);
        if (result && result.success) {
            console.log(`✅ Transfer completed email sent to ${sendingHospitalManager.email}`);
            return { success: true };
        } else {
            console.log(`⚠️ Could not send transfer completed email to ${sendingHospitalManager.email}`);
            return { success: false, error: 'SendGrid failed' };
        }
    } catch (error) {
...
const sendTransferRejectedEmail = async (requestingHospitalManager, fromHospital, toHospital, reason, requestId) => {
    try {
        ...
        const result = await sendViaSendGrid(requestingHospitalManager.email, `❌ Transfer Request Rejected - ${requestId}`, html);
        if (result && result.success) {
            console.log(`✅ Transfer rejected email sent to ${requestingHospitalManager.email}`);
            return { success: true };
        } else {
            console.log(`⚠️ Could not send transfer rejected email to ${requestingHospitalManager.email}`);
            return { success: false, error: 'SendGrid failed' };
        }
    } catch (error) {
...
const sendDonationEmail = async (email, name, donation) => {
    try {
        ...
        const result = await sendViaSendGrid(email, `Thank You for Your Donation Pledge - MediSync`, html);
        if (result && result.success) {
            console.log(`✅ Donation email sent to ${email}`);
            return { success: true };
        } else {
            console.log(`⚠️ Could not send donation email to ${email}`);
            return { success: false, error: 'SendGrid failed' };
        }
    } catch (error) {
...
const sendDonationApprovalEmail = async (email, name, donation, hospital) => {
    try {
        ...
        const result = await sendViaSendGrid(email, `✅ Your Donation Has Been Approved! - MediSync`, html);
        if (result && result.success) {
            console.log(`✅ Donation approval email sent to ${email}`);
            return { success: true };
        } else {
            console.log(`⚠️ Could not send donation approval email to ${email}`);
            return { success: false, error: 'SendGrid failed' };
        }
    } catch (error) {
...
const sendDonationRejectedEmail = async (email, name, donation, reason) => {
    try {
        ...
        const result = await sendViaSendGrid(email, `📋 Update on Your Donation - MediSync`, html);
        if (result && result.success) {
            console.log(`✅ Donation rejection email sent to ${email}`);
            return { success: true };
        } else {
            console.log(`⚠️ Could not send donation rejection email to ${email}`);
            return { success: false, error: 'SendGrid failed' };
        }
    } catch (error) {
...
const sendDonationCertificateEmail = async (email, name, donation, certificate) => {
    try {
        ...
        const attachments = certificate.filePath ? [{
            filename: certificate.fileName,
            content: certificate.fileContent || '',
            type: 'application/pdf'
        }] : [];

        const result = await sendViaSendGrid(email, `🎉 Your Donation Certificate - MediSync`, html, attachments);
        if (result && result.success) {
            console.log(`✅ Donation certificate email sent to ${email}`);
            return { success: true };
        } else {
            console.log(`⚠️ Could not send donation certificate email to ${email}`);
            return { success: false, error: 'SendGrid failed' };
        }
    } catch (error) {
...
const sendDonationNotificationToHospital = async (email, name, donation, donor) => {
    try {
        ...
        const result = await sendViaSendGrid(email, `📦 New Donation Assigned to Your Hospital - MediSync`, html);
        if (result && result.success) {
            console.log(`✅ Donation notification sent to hospital ${email}`);
            return { success: true };
        } else {
            console.log(`⚠️ Could not send donation notification to hospital ${email}`);
            return { success: false, error: 'SendGrid failed' };
        }
    } catch (error) {
...
const sendDonationCompletedNotification = async (email, name, donation, donor) => {
    try {
        ...
        const result = await sendViaSendGrid(email, `✅ Donation Completed - ${donation.donationId}`, html);
        if (result && result.success) {
            console.log(`✅ Donation completion notification sent to hospital ${email}`);
            return { success: true };
        } else {
            console.log(`⚠️ Could not send donation completion notification to ${email}`);
            return { success: false, error: 'SendGrid failed' };
        }
    } catch (error) {
...
const sendDonationUpdateEmail = async (adminEmail, adminName, donation, donorId, previousState) => {
    try {
        ...
        const result = await sendViaSendGrid(adminEmail, `✏️ Donation Updated - ${donation.donationId}`, html);
        if (result && result.success) {
            console.log(`✅ Donation update email sent to admin ${adminEmail}`);
            return { success: true };
        } else {
            console.log(`⚠️ Could not send donation update email to admin ${adminEmail}`);
            return { success: false, error: 'SendGrid failed' };
        }
    } catch (error) {
...
const sendDonationCancelledEmail = async (adminEmail, adminName, donation) => {
    try {
        ...
        const result = await sendViaSendGrid(adminEmail, `❌ Donation Cancelled - ${donation.donationId}`, html);
        if (result && result.success) {
            console.log(`✅ Donation cancelled email sent to admin ${adminEmail}`);
            return { success: true };
        } else {
            console.log(`⚠️ Could not send donation cancelled email to admin ${adminEmail}`);
            return { success: false, error: 'SendGrid failed' };
        }
    } catch (error) {
...
const sendDonationDeliveredEmail = async (adminEmail, adminName, donation, hospital, manager) => {
    try {
        ...
        const result = await sendViaSendGrid(adminEmail, `📦 Donation Delivered - ${donation.donationId}`, html);
        if (result && result.success) {
            console.log(`✅ Donation delivered email sent to admin ${adminEmail}`);
            return { success: true };
        } else {
            console.log(`⚠️ Could not send donation delivered email to admin ${adminEmail}`);
            return { success: false, error: 'SendGrid failed' };
        }
    } catch (error) {
